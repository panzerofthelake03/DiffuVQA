...
│   ├── vqa_model.py          #   TransformerNetModel, CVAE
...
**Training (single GPU):**
```bash
python train.py --dataset slake --data_dir datasets/slake --lr 0.00001 --batch_size 64 --learning_steps 150000 --seed 105 --noise_schedule sqrt --hidden_dim 768 --vocab bert --seq_len 64
```
...
2. **Question Encoder** (`diffuvqa/language_encoders/bert_model.py`) — BERT/PubMedBERT/BioBERT/RoBERTa encodes questions; controlled via `--vocab` parameter. Default: `bert`.
...
`--vocab` accepts: `bert`, `pubmedbert`, `biobert`, `roberta` — controls both the tokenizer and the language encoder weights loaded in `shared/basic_utils.py`. Default for this branch: `bert`.
