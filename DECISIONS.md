...
1. **Cevap uzunluk maskesi üretimi** — `input_ids_a` zaten fonksiyonda mevcut:
    ```python
    ans_len_mask = (input_ids_a != 0).float()  # [B, seq_len]
    ```

2. **MSE loss maskeli ortalamaya çevrildi** (eski: `mean_flat((ans_emb - ans_output)**2)`):
    ```python
    mse_per_token = ((x_start_mean - model_output_answer) ** 2).mean(dim=-1)  # [B, seq_len]
    mse_den = ans_len_mask.sum(dim=-1).clamp(min=1.0)
    terms["mse"] = (mse_per_token * ans_len_mask).sum(dim=-1) / mse_den
    ```

3. **t=0 özel loss (t0_loss) aynı maskeyle hesaplandı:**
    ```python
    t0_per_token = ((x_start_mean - model_out_x_start) ** 2).mean(dim=-1)
    t0_loss = (t0_per_token * ans_len_mask).sum(dim=-1) / ans_len_mask.sum(dim=-1).clamp(min=1.0)
    ```

4. **NLL loss'ları `_token_discrete_loss`'a mask geçildi** (`mask` parametresi zaten destekleniyor):
    ```python
...
- Mimariyi bozmaz; sadece loss hesabının semantiğini değiştirir.
- Padding tokenları gradient'ten çıkarılır, model kısa cevap için erken durma öğrenir.
- `clamp(min=1.0)` tamamen PAD gelen satırlarda sıfıra bölmeyi önler.

**Beklenen etki:**
- Eğitimde kısa cevap pozisyonlarındaki hatalı gradient kaldırıldı.
- Model zamanla daha temiz, kısa ve tutarlı cevaplar üretmeye başlar.
- Bu değişiklik davranış değişimi olduğu için yeniden eğitim veya fine-tune gerektirir.

---

### [BUGFIX] gaussian_diffusion.py — `_token_discrete_loss` NaN hatası (step 16)

**Hata:** `ans_len_mask` eklendikten sonra eğitimin 16. adımında `loss=nan` görüldü.

**Kök neden:** Bazı batch'lerde tüm cevap token'ları PAD olabilir (tamamen boş cevap satırı). Bu durumda `mask.sum(dim=-1) == 0` oluyor ve `_token_discrete_loss` içindeki bölme işlemi NaN üretiyor.

**Uygulanan fix (`_token_discrete_loss` fonksiyonu):**
```python
mask_sum = mask.sum(dim=-1).clamp(min=1.0)          # sıfıra bölmeyi önle
nll = (decoder_nll * mask).sum(dim=-1) / mask_sum   # maskeli ortalama
nll = th.where(mask.sum(dim=-1) == 0,               # tamamen PAD satır → loss=0
               th.zeros_like(nll), nll)
```

**Etki:** Step 16'dan itibaren loss stabil. Tamamen PAD cevap içeren satırlar loss'a katkıda bulunmuyor (sıfır gradyan), NaN yayılmıyor.
