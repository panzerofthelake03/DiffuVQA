"""
Architecture diagnostic tests for DiffuVQA — Bio-Bert branch.
...
FUSE_LEN = 16      # image patch count from FakeCLIP (controls fusion output length)
H       = 768      # hidden dim
...
class _FakeCLIP(nn.Module):
    """CPU-compatible stub that mimics CLIP ViT-B/32 output shape [B, 512, FUSE_LEN]."""
    def __init__(self):
        super().__init__()
    ...
    def forward(self, x):
        B = x.size(0)
        return torch.randn(B, 512, FUSE_LEN, device=x.device)
...
def build_diffusion(steps=100):
    ...
    args = make_args(diffusion_steps=steps)
...
def fake_cond(q_len=Q_LEN, a_len=A_LEN):
    ...
    tok = AutoTokenizer.from_pretrained(BIOBERT)
...
# ══════════════════════════════════════════════════════════════════════════════
# MODULE 2 — Text encoder (BioBERT question encoding)
# ══════════════════════════════════════════════════════════════════════════════
class TestTextEncoder:
    ...
    def test_question_encoding_no_nan(self):
        """Full question encoding through BioBERT: token+pos+type → LayerNorm → encoder."""
        cond = fake_cond()
        q_ids = cond["input_q_id"]
        ...
        assert not torch.isnan(emb).any(), "NaN after BioBERT embedding composition"
    ...
    def test_bert_encoder_frozen(self):
        """BioBERT encoder layers must be frozen (requires_grad=False)."""
        for p in self.model.fuse.bert.encoder.layer.parameters():
            assert not p.requires_grad, \
                "BioBERT encoder param is trainable — expected frozen for Bio-Bert branch"
...
class TestFeatureFusion:
    ...
    def test_output_shape(self):
        ...
        # fuse output length is determined by image patch count (FUSE_LEN)
        assert fuse_out.shape == (B, FUSE_LEN, H), \
            f"feature_fusion output shape wrong: {fuse_out.shape}, expected ({B}, {FUSE_LEN}, {H})"
        assert pre_ans.shape[0] == B and pre_ans.shape[2] == H, \
            f"pre_simu_answer shape wrong: {pre_ans.shape}"
        print(f"  fuse output: {fuse_out.shape}, pre_ans: {pre_ans.shape}")
...
class TestConcatenation:
    ...
    def test_get_ddpm_input_shape(self):
        ...
        assert fuse_feats.shape == (B, FUSE_LEN, H), \
            f"get_ddpm_input shape: {fuse_feats.shape}, expected ({B}, {FUSE_LEN}, {H})"
    ...
    def test_concat_shape(self):
        ...
        assert concat.shape == (B, FUSE_LEN + A_LEN, H), \
            f"Concat shape wrong: {concat.shape}, expected ({B}, {FUSE_LEN + A_LEN}, {H})"
        print(f"  fuse_len={FUSE_LEN}, ans_len={A_LEN}, total={FUSE_LEN+A_LEN}")
...
# ══════════════════════════════════════════════════════════════════════════════
# MODULE 6 — get_logits (embedding → vocab scores), exercises sqrt fix
# ══════════════════════════════════════════════════════════════════════════════
class TestGetLogits:
    ...
    def test_logits_no_nan(self):
        """Exercises the 1e-12 clamp fix — random embeddings must not produce NaN/Inf."""
        emb = torch.randn(B, A_LEN, H, device=DEVICE)
        with torch.no_grad():
            logits = self.model.get_logits(emb)
        assert not torch.isnan(logits).any(), "NaN in logits — sqrt(0) clamp fix may be missing"
        assert not torch.isinf(logits).any(), "Inf in logits"

    def test_correct_token_gets_best_logit(self):
        """If we feed the exact embedding of token X, logits must rank X first."""
        token_id = 1234  # arbitrary mid-vocab Bio-Bert token
        ...
        for b in range(B):
            assert top1[b].item() == token_id, \
                f"Exact embedding of token {token_id} not ranked first — " \
                f"argmax={top1[b].item()}. logits_mode=2 L2 distance broken or lm_head untied."
