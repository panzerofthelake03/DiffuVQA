"""
Architecture diagnostic tests for DiffuVQA — PubMedBERT branch.
...
PubMedBERT-specific invariants verified (not present in Bert branch):
  - BERT encoder.layer[0..11] are frozen (requires_grad=False)
  - BERT embeddings remain trainable
  - CLIP vision encoder is frozen
  - Loss formula is mse + nll + pre_answer_loss (no decoder_nll, no tT_loss)
...
def make_args(**overrides):
    args = types.SimpleNamespace(
        ...
        vocab="pubmedbert",
        vocab_size=30522,
        config_name="microsoft/BiomedNLP-BiomedBERT-base-uncased-abstract",
        seq_len=32,
        input_dims=768,
        output_dims=768,
        dropout=0.1,
...
def fake_cond(q_len=Q_LEN, a_len=A_LEN):
    # bert-base-uncased used as lightweight proxy: same WordPiece structure and
    # vocab_size=30522 as PubMedBERT. Shape tests do not require medical tokenisation.
...
# ══════════════════════════════════════════════════════════════════════════════
# MODULE 2 — Text encoder (PubMedBERT question encoding)
# ══════════════════════════════════════════════════════════════════════════════
class TestTextEncoder:
    ...
    def test_bert_encoder_layers_frozen(self):
        """All 12 BERT transformer layers must be frozen (PubMedBERT-specific freeze)."""
        for i, layer in enumerate(self.model.fuse.bert.encoder.layer):
            for name, param in layer.named_parameters():
                assert not param.requires_grad, \
                    f"BERT encoder.layer[{i}].{name} should be frozen but requires_grad=True"

    def test_bert_embeddings_trainable(self):
        """BERT embedding sub-modules cached in fuse must stay trainable.
        Note: bert.embeddings is deleted after init (vqa_model.py del temp_bert.embeddings)
        so we check the cached references on feature_fusion directly."""
        trainable_params = []
        for attr in ('position_embeddings', 'token_type_embeddings', 'bert_embedding_layer_norm'):
            mod = getattr(self.model.fuse, attr, None)
            if mod is not None:
                trainable_params.extend([p for p in mod.parameters() if p.requires_grad])
        assert len(trainable_params) > 0, \
            "All BERT embedding sub-modules are frozen — gradients cannot flow through embeddings"
...
class TestDiffusionLoss:
    ...
    def test_loss_ge_nll(self):
        """PubMedBERT loss = mse + nll + pre_answer_loss; all terms >= 0, so loss >= nll."""
        img    = fake_image()
        kwargs = self._make_model_kwargs()
        t      = torch.randint(1, 100, (B,), device=DEVICE)
        terms  = self.diffusion.training_losses_seq2seq(
        ...
        assert (terms["loss"] >= terms["nll"] - 1e-4).all(), \
            "loss < nll — mse or pre_answer_loss may be negative"

    def test_no_tT_loss_or_decoder_nll_key(self):
        """tT_loss and decoder_nll were removed in PubMedBERT — they must not appear in terms."""
        img    = fake_image()
        kwargs = self._make_model_kwargs()
        t      = torch.randint(1, 100, (B,), device=DEVICE)
        terms  = self.diffusion.training_losses_seq2seq(
            self.model, img, t, model_kwargs=kwargs)
        assert "tT_loss" not in terms, "tT_loss should have been removed from PubMedBERT loss"
        assert "decoder_nll" not in terms, "decoder_nll should have been removed from PubMedBERT loss"

    def test_model_kwargs_clean_after_call(self):
        """input_a_id and image_name must be popped from model_kwargs (no forward pollution)."""
        img    = fake_image()
        kwargs = self._make_model_kwargs()
        self.diffusion.training_losses_seq2seq(
            self.model, img,
            torch.randint(0, 100, (B,), device=DEVICE),
            model_kwargs=kwargs)
        assert "input_a_id" not in kwargs, \
            "input_a_id left in model_kwargs — causes TransformerNetModel.forward crash"
        assert "image_name" not in kwargs, \
            "image_name left in model_kwargs — causes TransformerNetModel.forward crash"
...
class TestEndToEndGradientFlow:
    ...
    def test_bert_encoder_layers_have_no_grad(self):
        """Frozen BERT transformer layers must not accumulate gradients."""
        img    = fake_image()
        cond   = fake_cond()
        kwargs = {
            "input_ids":  cond["input_ids"],
        ...
        for i, layer in enumerate(self.model.fuse.bert.encoder.layer):
            for name, param in layer.named_parameters():
                assert param.grad is None or (param.grad == 0).all(), \
                    f"Frozen BERT encoder.layer[{i}].{name} received a gradient"
