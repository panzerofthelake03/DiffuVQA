...
### Decision 12: Align training noising path with the non-leaky sampling default
- Files: diffuvqa/gaussian_diffusion.py, diffuvqa/config.json
- Change: Added runtime flag `use_noising_f` with default `false`.
- Change: In `training_losses_seq2seq`, the auxiliary noising branch now uses `f = cond_x_start` only when `use_noising_f=True`; otherwise `f=None` and `q_sample(..., add_information=False)` are used.
- Reason: Keep training semantics aligned with inference so the model does not rely on an answer-side shortcut that sampling does not provide.
- Concern: Historical checkpoints trained with the old shortcut are not directly comparable to the new default.
- Follow-up: Treat old and new runs as separate experiment families and keep the flag consistent when resuming.

### Decision 13: Make `pre_answer_loss` a controlled weighted ablation
- Files: diffuvqa/gaussian_diffusion.py, diffuvqa/config.json
- Change: Added runtime flag `pre_answer_loss_weight` with default `0.0`.
- Change: `pre_answer_loss` now contributes only when the weight is positive, with length alignment and answer masking applied before the loss is aggregated.
- Reason: Preserve clean generative training by default while still enabling a small supervised ablation when needed.
...
- Follow-up: Start with small values such as `0.05` or `0.1` if the ablation is needed.

### Decision 14: Expose and validate the new training objective controls in the Colab notebook
- File: shared/run_diffuvqa_colab.ipynb
- Change: Added `USE_NOISING_F` and `PRE_ANSWER_LOSS_WEIGHT` to the notebook config cell.
- Change: Passed both flags into the training command and extended resume compatibility checks to compare them against `training_args.json`.
- Reason: Prevent silent objective drift when launching or resuming runs from Colab.
- Concern: Mismatched resume attempts should fail fast instead of silently mixing objectives.
- Follow-up: Keep the notebook flags identical to the checkpoint metadata when resuming.

### Decision 15: Re-enable weight tying and add question residual fusion in Run C
- Files: diffuvqa/vqa_model.py
- Change: Added a `question_emb` residual path into the feature fusion output.
- Change: Re-tied `lm_head.weight` to `word_embedding.weight` when the shapes match.
- Reason: Strengthen the connection between question semantics and fusion output while keeping output projection aligned with the token embedding space.
- Concern: This changes the model family relative to earlier untied runs.
- Follow-up: Compare Run C checkpoints separately from the earlier untied baseline.
